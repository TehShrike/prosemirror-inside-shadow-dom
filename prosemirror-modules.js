// ../../common/temp/node_modules/.pnpm/w3c-keyname@2.2.8/node_modules/w3c-keyname/index.js
var base = {
  8: "Backspace",
  9: "Tab",
  10: "Enter",
...
// ../../common/temp/node_modules/.pnpm/orderedmap@2.1.1/node_modules/orderedmap/dist/index.js
...
// ../../common/temp/node_modules/.pnpm/prosemirror-model@1.19.3/node_modules/prosemirror-model/dist/index.js
...
var Slice = class _Slice {
  ...
  */
  static fromJSON(schema2, json) {
    ...
    return new _Slice(
      Fragment.fromJSON(schema2, json.content),
      openStart,
      openEnd
    );
  }
...
function removeRange(content, from2, to) {
  let { index, offset } = content.findIndex(from2),
  ...
  return content.replaceChild(
    index,
    child.copy(removeRange(child.content, from2 - offset - 1, to - offset - 1))
  );
}
...
var ResolvedPos = class _ResolvedPos {
  ...
  */
  marks() {
    ...
    for (var i = 0; i < marks2.length; i++)
      if (
        marks2[i].type.spec.inclusive === false &&
        (!other || !marks2[i].isInSet(other.marks))
      )
  ...
  */
  marksAcross($end) {
    ...
    let marks2 = after.marks,
      next = $end.parent.maybeChild($end.index());
    for (var i = 0; i < marks2.length; i++)
      if (
        marks2[i].type.spec.inclusive === false &&
        (!next || !marks2[i].isInSet(next.marks))
      )
        marks2 = marks2[i--].removeFromSet(marks2);
    return marks2;
  }
...
var Node2 = class _Node {
  ...
  */
  hasMarkup(type, attrs, marks2) {
    return (
      ...
      Mark.sameSet(this.marks, marks2 || Mark.none)
    );
  }
  /**
  ...
  */
  mark(marks2) {
    return marks2 == this.marks
      ? this
      : new _Node(this.type, this.attrs, this.content, marks2);
  }
  /**
...
var TextNode = class _TextNode extends Node2 {
  ...
  */
  ...
  mark(marks2) {
    return marks2 == this.marks
      ? this
      : new _TextNode(this.type, this.attrs, this.text, marks2);
  }
  withText(text) {
...
var NodeType = class _NodeType {
  ...
  */
  allowsMarks(marks2) {
    ...
    for (let i = 0; i < marks2.length; i++)
      if (!this.allowsMarkType(marks2[i].type)) return false;
    return true;
  }
  /**
  ...
  */
  static compile(nodes2, schema2) {
    let result = /* @__PURE__ */ Object.create(null);
    nodes2.forEach(
      (name, spec) => (result[name] = new _NodeType(name, schema2, spec))
    );
...
var MarkType = class _MarkType {
  ...
  */
  static compile(marks2, schema2) {
    ...
    marks2.forEach(
      (name, spec) =>
        (result[name] = new _MarkType(name, rank++, schema2, spec))
    );
    return result;
  }
  /**
...
var DOMParser = class _DOMParser {
  ...
  */
  static fromSchema(schema2) {
    return (
      schema2.cached.domParser ||
      (schema2.cached.domParser = new _DOMParser(
        schema2,
        _DOMParser.schemaRules(schema2)
      ))
    );
  }
};
...
var ParseContext = class {
  ...
  withStyleRules(dom, f) {
    ...
    if (!marks2) return;
    let [addMarks, removeMarks] = marks2,
      top = this.top;
    ...
    for (let i = 0; i < styles.length; i += 2) {
      for (let after = void 0; ; ) {
        ...
        if (rule.clearMark) {
          ...
          add3 = this.parser.schema.marks[rule.mark]
            .create(rule.attrs)
            .addToSet(add3);
        }
...
var DOMSerializer = class _DOMSerializer {
  ...
  */
  static fromSchema(schema2) {
    return (
      schema2.cached.domSerializer ||
      (schema2.cached.domSerializer = new _DOMSerializer(
        this.nodesFromSchema(schema2),
        this.marksFromSchema(schema2)
      ))
    );
  }
  /**
...
// ../../common/temp/node_modules/.pnpm/prosemirror-transform@1.7.4/node_modules/prosemirror-transform/dist/index.js
var lower16 = 65535;
  ...
  */
  static fromJSON(schema2, json) {
    ...
    return new _AddMarkStep(
      json.from,
      json.to,
      schema2.markFromJSON(json.mark)
    );
  }
};
  ...
  */
  static fromJSON(schema2, json) {
    ...
    return new _ReplaceStep(
      json.from,
      json.to,
      Slice.fromJSON(schema2, json.slice),
      !!json.structure
    );
  }
};
...
var Fitter = class {
      ...
      for (
        ...
        for (
          ...
          if (
            ...
            pass == 2 &&
            first &&
            (wrap2 = match.findWrapping(first.type))
          )
  ...
  placeNodes({ sliceDepth, frontierDepth, parent, inject, wrap: wrap2 }) {
    ...
    if (inject) {
      for (let i = 0; i < inject.childCount; i++) add3.push(inject.child(i));
  ...
  close($to) {
    ...
    for (let d = close2.depth + 1; d <= $to.depth; d++) {
      let node = $to.node(d),
        add3 = node.type.contentMatch.fillBefore(
          node.content,
          true,
          $to.index(d)
        );
...
// ../../common/temp/node_modules/.pnpm/prosemirror-state@1.4.3/node_modules/prosemirror-state/dist/index.js
...
// ../../common/temp/node_modules/.pnpm/prosemirror-keymap@1.2.2/node_modules/prosemirror-keymap/dist/index.js
var mac2 =
  typeof navigator != "undefined"
...
// ../../common/temp/node_modules/.pnpm/rope-sequence@1.3.4/node_modules/rope-sequence/dist/index.js
...
// ../../common/temp/node_modules/.pnpm/prosemirror-history@1.3.2/node_modules/prosemirror-history/dist/index.js
...
// ../../common/temp/node_modules/.pnpm/prosemirror-commands@1.5.2/node_modules/prosemirror-commands/dist/index.js
...
function markApplies(doc3, ranges, type) {
  for (let i = 0; i < ranges.length; i++) {
    ...
    let can =
      $from.depth == 0
        ? doc3.inlineContent && doc3.type.allowsMarkType(type)
        : false;
...
// ../../common/temp/node_modules/.pnpm/prosemirror-dropcursor@1.8.1/node_modules/prosemirror-dropcursor/dist/index.js
...
// ../../common/temp/node_modules/.pnpm/prosemirror-view@1.31.7/node_modules/prosemirror-view/dist/index.js
    ...
    for (let offset = 0, i = 0; i < this.children.length; i++) {
      ...
      if (
        ...
        if (from2 >= startInside && to <= endInside) {
          this.dirty =
            from2 == offset || to == end ? CONTENT_DIRTY : CHILD_DIRTY;
          if (
            from2 == startInside &&
            to == endInside &&
...
var MarkViewDesc = class _MarkViewDesc extends ViewDesc {
  ...
  slice(from2, to, view) {
    ...
    let nodes2 = this.children,
      size = this.size;
    if (to < size) nodes2 = replaceNodes(nodes2, to, size, view);
...
var ViewTreeUpdater = class {
  ...
  syncToMarks(marks2, inline, view) {
    ...
    while (
      ...
      marks2[keep].type.spec.spanning !== false
    )
      keep++;
    while (keep < depth) {
      this.destroyRest();
...
function selectVertically(view, dir, mods) {
  ...
  if ((sel instanceof TextSelection && !sel.empty) || mods.indexOf("s") > -1)
    return false;
  if (mac4 && mods.indexOf("m") > -1) return false;
  let { $from, $to } = sel;
  if (
    !$from.parent.inlineContent ||
...
function parseFromClipboard(view, text, html, plainText, $context) {
  ...
  if (asText) {
    ...
    if (parsed) {
      ...
      let { schema: schema2 } = view.state,
        serializer = DOMSerializer.fromSchema(schema2);
...
var inlineParents =
  /^([abiqsu]|abbr|acronym|cite|code|del|em|ins|kbd|label|output|ruby|samp|span|strong|sub|sup|time|tt|var)$/i;
...
function restoreReplacedSpaces(dom) {
  let nodes2 = dom.querySelectorAll(
...
function addContext(slice2, context) {
  ...
  let schema2 = slice2.content.firstChild.type.schema,
    array;
  try {
...
var brokenClipboardAPI =
  (ie2 && ie_version < 15) || (ios && webkit_version < 604);
...
function safariShadowSelectionRange(view) {
  ...
  let anchorNode = found2.startContainer,
    anchorOffset = found2.startOffset;
  let focusNode = found2.endContainer,
    focusOffset = found2.endOffset;
...
var isInline =
  /^([abiqsu]|abbr|acronym|bd[io]|big|br|button|cite|code|data(list)?|del|dfn|em|ins|kbd|label|map|mark|meter|output|ruby|samp|small|span|strong|su[bp]|time|tt|var)$/i;
  ...
  if (parse.sel) {
    ...
    if (
      ...
      !(
        ...
        (ie2 && sel2.empty && sel2.head == chFrom)
      )
    )
...
// ../../common/temp/node_modules/.pnpm/prosemirror-gapcursor@1.3.2/node_modules/prosemirror-gapcursor/dist/index.js
...
// ../../common/temp/node_modules/.pnpm/crelt@1.0.6/node_modules/crelt/index.js
function crelt() {
  var elt = arguments[0];
...
// ../../common/temp/node_modules/.pnpm/prosemirror-menu@1.2.2/node_modules/prosemirror-menu/dist/index.js
...
function isIOS() {
  ...
  return (
    !/Edge\/\d/.test(agent2) &&
    /AppleWebKit/.test(agent2) &&
    /Mobile\/\w+/.test(agent2)
  );
}
...
// ../../common/temp/node_modules/.pnpm/prosemirror-schema-list@1.3.0/node_modules/prosemirror-schema-list/dist/index.js
var olDOM = ["ol", 0];
var ulDOM = ["ul", 0];
var liDOM = ["li", 0];
var orderedList = {
...
function add2(obj, props) {
  let copy2 = {};
  for (let prop in obj) copy2[prop] = obj[prop];
  for (let prop in props) copy2[prop] = props[prop];
  return copy2;
}
function addListNodes(nodes2, itemContent, listGroup) {
  return nodes2.append({
    ordered_list: add2(orderedList, {
      content: "list_item+",
      group: listGroup,
    }),
...
function wrapInList(listType, attrs = null) {
  return function (state, dispatch) {
    ...
    if (dispatch)
      dispatch(
        doWrapInList(state.tr, range, wrap2, doJoin, listType).scrollIntoView()
      );
    return true;
  };
}
...
// ../../common/temp/node_modules/.pnpm/prosemirror-inputrules@1.2.1/node_modules/prosemirror-inputrules/dist/index.js
var InputRule = class {
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    let tr =
      ...
      rules[i].handler(
        state,
        match,
        from2 - (match[0].length - text.length),
        to
      );
    if (!tr) continue;
      ...
      if (dispatch) {
        ...
        if (undoable.text) {
          ...
          tr.replaceWith(
            undoable.from,
            undoable.to,
            state.schema.text(undoable.text, marks2)
          );
        } else {
...
// ../../common/temp/node_modules/.pnpm/prosemirror-example-setup@1.2.2/node_modules/prosemirror-example-setup/dist/index.js
var prefix2 = "ProseMirror-prompt";
function openPrompt(options) {
  let wrapper = document.body.appendChild(document.createElement("div"));
  wrapper.className = prefix2;
  let mouseOutside = (e) => {
    if (!wrapper.contains(e.target)) close2();
  };
  ...
  form.addEventListener("keydown", (e) => {
    if (e.keyCode == 27) {
      ...
      window.setTimeout(() => {
        if (!wrapper.contains(document.activeElement)) close2();
      }, 500);
    }
  });
  ...
  return new MenuItem({
    ...
    run(state, _, view) {
      ...
      openPrompt({
        ...
        callback(attrs2) {
          view.dispatch(
            view.state.tr.replaceSelectionWith(nodeType.createAndFill(attrs2))
          );
          view.focus();
        },
      });
...
function markActive(state, type) {
  ...
  if (empty2) return !!type.isInSet(state.storedMarks || $from.marks());
  else return state.doc.rangeHasMark(from2, to, type);
}
...
function buildMenuItems(schema2) {
  ...
  if ((mark = schema2.marks.link)) r.toggleLink = linkItem(mark);
  let node;
  if ((node = schema2.nodes.image)) r.insertImage = insertImageItem(node);
...
function buildKeymap(schema2, mapKeys) {
  ...
  if ((type = schema2.marks.code)) bind2("Mod-`", toggleMark(type));
  ...
  if ((type = schema2.nodes.blockquote)) bind2("Ctrl->", wrapIn(type));
...
// ../../common/temp/node_modules/.pnpm/prosemirror-schema-basic@1.2.2/node_modules/prosemirror-schema-basic/dist/index.js
var pDOM = ["p", 0];
...
export { EditorState, EditorView, Schema, addListNodes, exampleSetup, schema };
